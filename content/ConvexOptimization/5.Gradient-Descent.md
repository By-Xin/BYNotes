---
title: "Lecture 05 - Gradient Descent"
aliases:
  - "5. Gradient Descent"
  - "Gradient Descent"
  - ConvexOptimization/05_GradientDescent
...
> [!proof]+ Proof of strong convexity lower bound
...
利用该不等式, 我们可以有效地分析梯度的大小对于优化值与最优值差距 $f(x)-p^\star$ 的影响, 分析如下.
...
> [!proof]+ Proof of descent direction condition
> 根据凸性的 supporting hyperplane 定理, 对于任意 $x, x^+$, 有 $f(x^+) - f(x) \geq \nabla f(x)^\top (x^+ - x) = t \nabla f(x)^\top \Delta x$. 由于 Descent Method 要求 $f(x^+) < f(x)$ 对任意 $t > 0$ 成立, 故 $\nabla f(x)^\top \Delta x < 0$.
>
> $\square$
...
- 若再进一步结合收敛准则 $f(x^{(k)}) - p^\star < \epsilon$, 即令 $c^k (f(x^{(0)}) - p^\star) \leq \epsilon$, 可得 GD 的迭代次数 $k$ 满足:
  ...
  $$
  k \geq \frac{\log\left(\frac{f(x^{(0)}) - p^\star}{\epsilon}\right)}{\log\left(\frac{1}{c}\right)} = \frac{\log\left(\frac{f(x^{(0)}) - p^\star}{\epsilon}\right)}{\log\left(\frac{M}{M-m}\right)}.
  $$
...
- **Claim.** 只要通过算法将步长优化至 $0\leq t \leq 1/M$, 则 Armijo 条件必然满足.
  ...
  > [!proof]+ Proof of the Armijo condition claim
...
- [[3.Convexity-II|由凸性的 Supporting Hyperplane 定理]]:
...
如下定理说明任意一阶方法的收敛速率下界为 $\mathcal{\Omega}(1/k^2)$.
