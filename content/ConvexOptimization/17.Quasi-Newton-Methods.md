---
title: "Lecture 17 - Quasi-Newton Methods"
aliases:
  - "17. Quasi-Newton Methods"
  - "Quasi-Newton Methods"
  - "QuasiNewton"
  - ConvexOptimization/17_QuasiNewton
...
> $$
> 因此, 自然地会要求这个近似的结果在 $\mathbf{x}^{k}$ 和 $\mathbf{x}^{k+1}$ 处的梯度与原函数 $f(\mathbf{x})$ 的梯度相匹配:
> $$
...
> [!algorithm] ALGORITHM: Generic Quasi-Newton Method
...
>    1. **COMPUTE** 搜索方向 $\mathbf{d}^k = -(B^k)^{-1} \nabla f(\mathbf{x}^k)$ (或 $\mathbf{d}^k = -H^k \nabla f(\mathbf{x}^k)$).
>    2. **COMPUTE** 步长 $\alpha_k > 0$ 通过线搜索确定.
>    3. **UPDATE** $\mathbf{x}^{k+1} \leftarrow \mathbf{x}^k + \alpha_k \mathbf{d}^k$.
>    4. **UPDATE** Hessian 近似 $B^{k+1}$ (或 $H^{k+1}$) 使其满足 secant equation.
...
- 因此, 最后通过迭代得到的 $\mathbf{r}_k$ 就是 $H^k \mathbf{g}^k$ 的值, 恰恰就是下一步的搜索方向 (之相反数) $\mathbf{d}^k = -H^k \mathbf{g}^k = -\mathbf{r}_k$.

以上就构成了 L-BFGS 方法中计算搜索方向的核心算法. 其一共需要约 $4mn$ 次乘法与 $2mn$ 次加法运算. 总的计算复杂度约为 $\mathcal{O}(mn)$.
...
然而这个式子事实上是需要知道 $H^{k-m}$ 的值的. 该取值本身在第一阶段没有涉及, 但在第二阶段的初始迭代中, 便需要计算:
$$
...
$$
来近似 $H^{k-m}$ 的值. 这一选择本身也是合理的. 因为, $H^{k}$ 本身作为 inverse Hessian 的近似, 我们期望其能够满足关系:
$$
...
> [!algorithm] ALGORITHM: L-BFGS 双循环递推算法
> **INPUT:** 当前梯度 $\mathbf{g}^k=\nabla f(\mathbf{x}^k)\in\mathbb{R}^n$；最近 $m$ 对历史曲率信息 $\{(\mathbf{s}^j,\mathbf{y}^j)\}_{j=k-m}^{k-1}$；标量 $\rho_j=\frac{1}{(\mathbf{y}^j)^\top\mathbf{s}^j}$；初始逆 Hessian 近似矩阵 $H_0^k$.
>
> **OUTPUT:** $\mathbf{r}_k$, 即 $H^k \mathbf{g}^k$ 的值.
>
> 1. **SET** $\mathbf{q} \leftarrow \nabla f(\mathbf{x}^k)$.
> 2. **FOR** $j=k-1, k-2, \cdots, k-m$ **DO**
>    1. **COMPUTE** 并保存 $\alpha_j \leftarrow \rho_j \langle \mathbf{s}^j, \mathbf{q} \rangle$.
>    2. **UPDATE** $\mathbf{q} \leftarrow \mathbf{q} - \alpha_j \mathbf{y}^j$.
> 3. **SET** $\mathbf{r} \leftarrow \hat{H}_0^{k-m} \mathbf{q}$. 其中 $\hat{H}_0^{k-m}$ 是对 $H^{k-m}$ 的近似, 可以选取 $\hat{H}_0^{k-m} = \gamma_{k-m} I$.
> 4. **FOR** $j=k-m, k-m+1, \cdots, k-1$ **DO**
>    1. **COMPUTE** $\beta_j \leftarrow \rho_j \langle \mathbf{y}^j, \mathbf{r} \rangle$.
>    2. **UPDATE** $\mathbf{r} \leftarrow \mathbf{r} + \mathbf{s}^j (\alpha_j - \beta_j)$.

接着, 在给出了搜索方向 $\mathbf{d}^k$ 之后, 使用 L-BFGS 进行优化迭代的完整算法如下所示.

> [!algorithm] ALGORITHM: L-BFGS 优化算法
> **INPUT:** 初始点 $\mathbf{x}^0\in\mathbb{R}^n$；历史曲率信息的记忆长度 $m$.
>
> **OUTPUT:** 最优解 $\mathbf{x}^*$.
>
> 1. **FOR** $k=0,1,2,\cdots$ **DO**
>    1. **SET** 近似矩阵 $\hat{H}_0^{k-m}$.
>    2. **COMPUTE** 搜索方向 $\mathbf{d}^k \leftarrow -H^k \nabla f(\mathbf{x}^k)$ 使用双循环递推算法.
>    3. **COMPUTE** 步长 $\alpha_k$ 使用 Wolfe 条件线搜索.
>    4. **UPDATE** $\mathbf{x}^{k+1} \leftarrow \mathbf{x}^k + \alpha_k \mathbf{d}^k$.
>    5. **IF** $k \geq m$ **THEN**
>       1. **UPDATE** 历史集合: 丢弃最早的历史曲率信息 $(\mathbf{s}^{k-m}, \mathbf{y}^{k-m})$.
>    6. **COMPUTE** 新的历史曲率信息 $(\mathbf{s}^k, \mathbf{y}^k)$, 并 **UPDATE** 历史集合: 将其加入历史曲率信息集合中.
>    7. **IF** 收敛条件满足 **THEN** **BREAK**.
