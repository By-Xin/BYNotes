...
### Decomposable Functions (Composite Objective)
...
- $\phi$ 是光滑凸函数, 不妨令 $\text{dom}(\phi) = \mathbb{R}^n$. (更一般地, 只需 $\phi$ 在算法迭代点所在区域可微且 $\nabla\phi$ 满足 Lipschitz 条件; 若 $\phi$ 的定义域带来约束, 可将对应指示函数并入 $h$.)
...
$$
\boxed{
x^+ := \arg\min_{z} Q_t(z,x),
\qquad
Q_t(z,x):=\phi(x)+\nabla\phi(x)^\top(z-x)+\frac{1}{2t}\|z-x\|^2+h(z).
}
$$

对 $Q_t(z,x)$ 做配方并去掉与 $z$ 无关项, 可得

$$
\boxed{
x^+
=
\arg\min_{z}\left[h(z)+\frac{1}{2t}\left\|z-(x-t\nabla\phi(x))\right\|^2\right]
=
\mathrm{prox}_{t h}(x-t\nabla\phi(x)).
}
$$

- 这个更新步骤的核心思想是: 在每次迭代中, 首先对光滑部分 $\phi$ 进行梯度下降的更新, 得到一个临时变量 $x' = x - t \nabla \phi(x)$; 然后通过最小化包含非光滑部分 $h$ 的二次近似目标函数来得到新的参数 $x^+$.
...
***Definition* (近端算子):** 对于一个凸函数 $h: \mathbb{R}^n \to (-\infty, +\infty]$ (proper, closed, convex), 其近端算子 $\text{prox}_{h}: \mathbb{R}^n \to \mathbb{R}^n$ 定义为:
...
- **$u = \text{prox}_{h}(x)$ 等价于 $x-u \in \partial h(u)$, 其中 $\partial h(u)$ 是 $h$ 在 $u$ 处的次微分.**
  ...
  - *Proof*
    - 若已知 $u = \text{prox}_{h}(x)$, 则 $u$ 是以下优化问题的最小值: $ \min_{v} \psi(v) := \frac{1}{2} \|v - x\|^2 + h(v)$. 因此 $\mathbf{0} \in \partial \psi(u) = u - x + \partial h(u)$, 从而 $x-u \in \partial h(u)$
    - 若 $x-u \in \partial h(u)$, 则由次梯度定义可知 $h(v) \geq h(u) + (x-u)^\top (v-u)$, 从而

      $$
      h(v)+ \frac{1}{2} \|v-x\|^2 \ge h(u) + (x-u)^\top (v-u) + \frac{1}{2}\|v-x\|^2 = h(u) + \frac{1}{2}\|u-x\|^2 + \frac{1}{2}\|v-u\|^2 \ge h(u) + \frac{1}{2}\|u-x\|^2,
      $$
...
***Definition* (缩放的近端算子):** 对于一个凸函数 $h: \mathbb{R}^n \to (-\infty,+\infty]$ 以及一个正标量 $t>0$, 其缩放的近端算子 $\text{prox}_{t h}: \mathbb{R}^n \to \mathbb{R}^n$ 定义为:

$$
\boxed{\text{prox}_{t h}(x) = \arg\min_{z} \left[h(z) + \frac{1}{2t} \|z - x\|^2\right]}
$$

- 该算子与未缩放的近端算子类似, 但在优化目标中对函数 $h$ 进行了缩放, 这在实际应用中可以调整近端映射的强度.
- 与 $\arg\min_{z}\left[\frac{1}{2}\|z-x\|^2 + t h(z)\right]$ 的写法完全等价 (只差一个正数倍缩放).
...
- 事实上, 对于含约束的优化问题同样也可以令 $h(x) = \delta_{\mathcal{C}}(x)$ (其中 $\mathcal{C}$ 为非空闭凸集) 来将约束条件隐式地包含在非光滑函数中, 从而使得近端梯度下降算法同样适用.
...
- 其中第三个等式中
  ...
  - Proximal Gradient Mapping 与次梯度的关系为:
    ...
    $$
    \boxed{G_{t_k}(x^{(k)}) - \nabla \phi(x^{(k)}) \in \partial h(x^{(k+1)})=\partial h\left(x^{(k)} - t_k G_{t_k}(x^{(k)})\right)} \quad (\dagger)
    $$
...
- 该优化问题同样可以通过 proximal gradient descent 方法来求解:
  ...
  - $\text{prox}_{t_k h}(Y) = U \cdot \text{Diag}((d - t_k \mu)_+) \cdot V^\top$, 其中 $Y = U \cdot \text{Diag}(d) \cdot V^\top$ 是 $Y$ 的奇异值分解(SVD), 且 $(a)_+ := \max\{a,0\}$ 对向量逐元素作用.
- 因此, proximal gradient descent 的迭代更新步骤为:

$$
\begin{aligned}
Y^{(k)} &= X^{(k)} - t_k \nabla \phi(X^{(k)}) = X^{(k)} - t_k\big(P \odot (X^{(k)} - M)\big),\\
Y^{(k)} &= U^{(k)} \cdot \text{Diag}(d^{(k)}) \cdot (V^{(k)})^\top,\\
X^{(k+1)} &= U^{(k)} \cdot \text{Diag}\big((d^{(k)} - t_k \mu)_+\big) \cdot (V^{(k)})^\top.
\end{aligned}
$$
...
整体而言, 给定 $\rho \in (0,1)$ 及初始化 $t_0 > 0$, backtracking line search 的步骤如下:
- 先计算

  $$
  x^+(t)=\text{prox}_{t h}(x - t\nabla\phi(x)),\qquad G_t(x)=\frac{x-x^+(t)}{t}.
  $$

- 检查条件

  $$
  \phi(x - t \cdot G_t(x)) \le \phi(x) + (\nabla \phi(x))^\top (-t \cdot G_t(x)) + \frac{1}{2t} \|t \cdot G_t(x)\|^2.
  $$

- 若不满足, 则令 $t \leftarrow \rho t$, 并重新计算 $x^+(t)$ 与 $G_t(x)$ 后再次检查.
- 等价地, 也可写成更常见的上界模型检验: $F(x^+(t)) \le Q_t(x^+(t),x)$.
  ...
  F(x^+(t)) \le Q_t(x^+(t),x)
  \iff
  \phi(x^+(t)) \le \phi(x)+\nabla\phi(x)^\top(x^+(t)-x)+\frac{1}{2t}\|x^+(t)-x\|^2.
  $$

在搜索过程中, 由于 $G_t(x)$ 本身依赖于 $t$, 每次缩步长都必须重新计算 prox 映射.
...
1. $\phi(x^{(k)}) \leq \phi(y^{(k)}) + \langle \nabla \phi(y^{(k)}), x^{(k)} - y^{(k)} \rangle + \frac{1}{2 t_k} \|x^{(k)} - y^{(k)}\|^2_2$
...
$$
t_{k-1}\gamma_k^2=t_k\gamma_{k-1}^2(1-\gamma_k),
$$

这是关于 $\gamma_k$ 的二次方程. 通过求解该方程来动态调整 $\gamma_k$ 和 $t_k$, 可以在保证满足 line search 条件的同时, 使得步长 $t_k$ 不会过快地缩小, 从而提升算法的收敛效率.

#### Several Remarks on FISTA
