---
title: "Lecture 23 - Frank-Wolfe Method"
aliases:
  - "23. Frank-Wolfe Method"
  - "Frank-Wolfe Method"
  - "Frank-Wolfe"
...
回顾 Projected Gradient Descent 和 [[8.Proximal-Gradient-Descent|Proximal Gradient Descent 方法]]. 给定如下一般有约束问题:
$$
...
### 1.3 Frank-Wolfe in Norm Constraints
...
### 1.4 Constrained and Lagrange forms
...
其基本等价于一个贪心的 [[20.Coordinate-Descent|coordinate descent 方法]], 每次迭代只更新一个坐标 (贪心地选择梯度最大的坐标), 并且每次迭代的步长逐渐减小.

### 2.2 lp Regularization
...
[[6.Subgradient|下具体求解该函数的次梯度]].

- **Lemma 1.** 给定凸集 $\mathcal{B}$, 设 $\phi(z) = \max_{u \in \mathcal{B}} \langle u, z \rangle$. 若 $u^* \in \arg\max_{u \in \mathcal{B}} \langle u, z_0 \rangle$, 则有 $u^* \in \partial \phi(z_0)$.

    > [!proof]+ Proof
    >
    > 次梯度需证 $\phi(z) \geq \phi(z_0) + \langle u^*, z - z_0 \rangle$ 对任意 $z$ 成立. 由 $u^*$ 的最优性, $\phi(z_0) = \langle u^*, z_0 \rangle$. 故 $\phi(z_0) + \langle u^*, z - z_0 \rangle = \langle u^*, z \rangle \leq \max_{u \in \mathcal{B}} \langle u, z \rangle = \phi(z)$, 证毕.
...
对于 Frank-Wolfe 方法, 其步长设置为 $\gamma_k = 2/(k+2)$ 时, 有如下收敛性结论:
$$
...
#### 4.3.2 Fully Corrective Frank-Wolfe
...
因此, 若在 $t_{k-1}$ 处求得的近似解 $\hat{x}(t_{k-1})$ 满足精度要求 $g_{t_{k-1}}(\hat{x}(t_{k-1})) \leq \varepsilon/m$, 且在 $t_{k-1} +\Delta t$ 处恰超过 $\varepsilon$, 即:
$$
g_{t_{k-1} + \Delta t}(\hat{x}(t_{k-1})) = g_{t_{k-1}}(\hat{x}(t_{k-1})) + \Delta t \|\nabla f(\hat{x}(t_{k-1}))\|_* = \varepsilon
$$
从中可以反过来解出, 在
$$
