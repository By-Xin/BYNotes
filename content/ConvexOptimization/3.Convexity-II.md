---
title: "Lecture 03 - Convexity II: Convex Optimization Problems"
aliases:
  - "3. Convexity II: Convex Optimization Problems"
  - "Convexity II"
  - "Convexity (II): Convex Optimization Problems"
  - ConvexOptimization/03_Convexity(II)
...
- 其中 $f, g_i$ 是凸函数, optimization domain $D = \bigcap_{i=1}^m \text{dom}(g_i) \cap \text{dom}(f)$ 是凸集. $f$ 称作 criterion 或 objective function. $g_i$ 称作 inequality constraint functions.
...
- 若存在 $x^*$ 是所有 feasible point 中使 $f(x)$ 取得最小值的点, 则称 $x^*$ 为 optimal point 或该问题的 solution, 最小值 $f(x^*)$ 称作 optimal value. 若 $x$ 是 feasible point, 且有 $f(x) \leq p^\star+\epsilon$, 则称 $x$ 为 $\epsilon$-suboptimal point.
...
- 若优化问题有解, 则记所有最优解组成的集合为 $X_{\text{opt}}$:
  ...
  $$
  X_{\text{opt}} = \{x \mid g_i(x) \leq 0, \forall i~; Ax=b; f(x)=p^\star\}
  $$
...
- 若对于某个 feasible point $x_0$, 存在某个 $R>0$, 满足
  ...
  $$
  f(x_0)  = \inf \{f(z) \mid g_i(z) \leq 0, \forall i~; A z=b; \|z - x_0\|_2 \leq R\}
  $$
...
1. 由于 $f$ 和 $g_i$ 均为凸函数, 且约束条件均为凸集, 因此 $X_{\text{opt}}$ 也是凸集.
   ...
   > [!proof]+ Proof of convexity of the optimal solution set
   >
   ...
   >   - $f(\theta x_1 + (1-\theta)x_2) \leq \theta f(x_1) + (1-\theta)f(x_2) = p^\star$, 因此 $\theta x_1 + (1-\theta)x_2$ 也是最优解.
...
2. 若 $f$ 严格凸, 则最优解若存在则唯一. 故可立刻推出任意局部最优解也是全局最优解.
   ...
   > [!proof]+ Proof of uniqueness under strict convexity
   >
   > - 假设存在 $x_1, x_2 \in X_{\text{opt}}$, 且 $x_1 \neq x_2$, 则对任意 $\theta \in (0,1)$, 有:
   >   - $f(\theta x_1 + (1-\theta)x_2) < \theta f(x_1) + (1-\theta)f(x_2) = p^\star$, 这与 $p^\star$ 为最小值矛盾.
   > - 因此, 最优解唯一.
   >
   > $\square$
...
- 若 $n\ge p$ 且 $\boldsymbol{X}$ 是列满秩的, 则该问题的解是唯一的.
  ...
  > [!proof]+ Proof of uniqueness in the full-column-rank case
  >
...
**[[Support-Vector-Machine|Example (SVM)]].** 考虑如下支持向量机(SVM)的优化问题. 对于给定的训练数据集 $\{(\boldsymbol{x}_i, y_i)\}_{i=1}^n$, 其中 $\boldsymbol{x}_i \in \mathbb{R}^p$ 是样本特征, $y_i \in \{-1, 1\}$ 是样本标签, SVM 的优化问题定义为 (其中 $\boldsymbol{\xi} = (\xi_1, \ldots, \xi_n)^\top$ 是松弛变量, 允许一定程度的分类错误):
...
<img src="https://raw.githubusercontent.com/By-Xin/Blog-figs/main/20260216164456.png" alt="First-order optimality condition illustration" style="display: block; width: 50%; height: auto; margin: 1.25rem auto;">

> [!proof]+ Proof of the first-order optimality condition
>
...
>   - 由 $g'(0) < 0$ 和导数定义可知, 存在足够小的 $t_0 > 0$ 使得 $g(t_0) < g(0) = f(x^*)$. 故矛盾.
...
> [!proof]+ Proof of the unconstrained case
>
> 下说明退化之方式.
...
> [!proof]+ Proof of the equality-constrained case
>
...
>   $$
>
> - 由于 $\mathbf{y},\mathbf{x}^*$ 同时满足等式约束, 故 $A(\mathbf{y} - \mathbf{x}^*) := A\mathbf{u} = 0$. 这说明, 所有的可行位移都应处在 $A$ 的零空间内, 即 $\mathbf{u} \in \text{Nul}(A) = \{\mathbf{u} \mid A\mathbf{u} = 0\}$. 换言之, 所有可行的 $\mathbf{y}$ 都应满足 $\mathbf{y} = \mathbf{x}^* + \mathbf{u}, \mathbf{u} \in \text{Nul}(A)$, 即 $\mathbf{y}$ 和 $\mathbf{u}$ 是一一对应的, 故最优性条件可以改写为:
>
>   $$
>   \begin{aligned}
...
> [!proof]+ Proof of convexity under partial minimization
>
...
- 特别地, 对于凸但非 affine 的等式约束 $h_j(x) = 0$, 我们可以考虑将其放松为 $h_j(x) \leq 0$, 以确保凸性.
