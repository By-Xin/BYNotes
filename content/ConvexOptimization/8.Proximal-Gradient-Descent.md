---
title: "Lecture 08 - Proximal Gradient Descent"
aliases:
  - "8. Proximal Gradient Descent"
  - "Proximal Gradient Descent"
  - ConvexOptimization/08_ProximalGradientDescent
...
在处理这样的优化问题时传统的梯度下降方法可能无法直接应用, 因为 $h$ 的非光滑性会导致梯度不存在; 而[[7.Subgradient-Methods|若直接使用次梯度方法]], 则可能会导致收敛速度较慢.

因此我们需要一种新的优化方法来处理这种分解结构的目标函数, 这就是**近端梯度下降**(Proximal Gradient Descent) 方法. 其对于光滑的部分 $\phi$ 使用梯度信息, 对于非光滑的部分 $h$ 则利用其近端算子来进行优化.

回顾一下, [[5.Gradient-Descent|传统的梯度下降方法在每一步迭代中更新参数]] $x$ 的方式为: $x^+ = x - t \Delta x$. 我们选择 $\Delta x = \nabla f(x)$, 是因为对 $f(z)$ 在 $x$ 处做一阶近似并加上二次正则后, 有:
...
- **$u = \text{prox}_{h}(x)$ 等价于 $x-u \in \partial h(u)$, 其中 $\partial h(u)$ 是 $h$ 在 $u$ 处的次微分.**
  ...
  > [!proof]+ Proof of the prox/subgradient equivalence
...
> [!proof]+ Proof of the convergence rate
...
<img src="https://raw.githubusercontent.com/By-Xin/Blog-figs/main/20260214003311.png" alt="Projected gradient descent" width="420">
