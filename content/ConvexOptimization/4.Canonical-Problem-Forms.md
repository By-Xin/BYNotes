---
title: "Lecture 04 - Canonical Problem Forms"
aliases:
  - "4. Canonical Problem Forms"
  - "Canonical Problem Forms"
  - ConvexOptimization/04_CanonicalProblems
...
& && \mathbf{x}^+, \mathbf{x}^-, \mathbf{s} \geq \mathbf{0}
\end{aligned}
$$
...
$$
\begin{aligned}
\min_x &\quad && \mathbf{c}^\top \mathbf{x} + \frac{1}{2} \mathbf{x}^\top \mathbf{Q} \mathbf{x} \\
\text{s.t.} &\quad && D\mathbf{x} \leq \mathbf{d} \\
&\quad && A\mathbf{x} = \mathbf{b}
\end{aligned}
$$

- 其中 $\mathbf{Q} \succeq 0$ 是半正定矩阵, 以确保目标函数是凸的.
...
**[[IPMO-Integrated-Prediction-and-Multi-period-Portfolio-Optimization|Example (Portfolio Optimization)]].** 给定 $n$ 种资产, 其预期收益率为 $\mu \in \mathbb{R}^n$, 协方差矩阵为 $\Sigma \in \mathbb{R}^{n \times n}$, 投资组合权重为 $\mathbf{x} \in \mathbb{R}^n$. 投资组合优化问题可以表示为:
...
**[[Support-Vector-Machine|Example (Support Vector Machine)]].** 支持向量机 (SVM) 的训练问题也可以表示为凸二次规划. 给定训练数据集 $\{(\mathbf{x}_i, y_i)\}_{i=1}^n$, 其中 $\mathbf{x}_i \in \mathbb{R}^d$ 是特征向量, $y_i \in \{-1, 1\}$ 是标签. SVM 的优化问题为:
...
**[[Regularization|Example (LASSO)]].** LASSO (Least Absolute Shrinkage and Selection Operator) 回归问题也可以转化为凸二次规划. 给定数据矩阵 $X \in \mathbb{R}^{m \times n}$ 和响应向量 $\mathbf{y} \in \mathbb{R}^m$, LASSO 的优化问题为:
...
- 对于未知量 $\mathbf{x} \in \mathbb{R}^n$, 可以观测到线性测量 $\mathbf{y} = A\mathbf{x}$, 其中 $A \in \mathbb{R}^{m \times n}, m \ll n$ 是已知的测量矩阵, 此时方程有无穷多解. 额外引入稀疏的先验假设, 故优化目标为 $\min \|\mathbf{x}\|_0, \text{ s.t. } A\mathbf{x} = \mathbf{y}$. 该问题是 NP-hard 的, 因此可以使用凸的 $\ell_1$ 范数作为松弛.
