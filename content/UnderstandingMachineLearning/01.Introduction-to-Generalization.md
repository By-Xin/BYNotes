---
title: "01 - Introduction to Generalization"
aliases:
...
> - Video: <https://www.bilibili.com/video/BV1k64y1r7Dv/?spm_id_from=333.1387.homepage.video_card.click&vd_source=8a00dab0be94d29388f2286892ba8d50>

## 1. A Formal Model - The Statistical Learning Framework
...
- 我们期望 learner 能够学习一种预测规则 (称 predictor / hypothesis / classifier) $h: \mathcal{X} \to \mathcal{Y}$ 能够给定 feature $\mathbf{x}$, 尽可能准确地预测 label $y$. 此外, 引入 **algorithm** $\mathcal{A}$, 其负责从训练集 $S$ 中学习 predictor $h$. 也就是说, $\mathcal{A}$ 是一个 mapping: $\mathcal{A}: S \to h$.
...
- 以 0-1 二分类问题为例, 我们定义 prediction rule 的 loss function 为:
    ...
    $$
    - 直观地讲, $L_{\mathcal{D}, f}(h)$ 表示 predictor $h$ 在 distribution $\mathcal{D}$ 下的 population loss, 即 predictor $h$ 在所有可能的 feature 上预测错误的概率. 第二个等式相当于以测度的视角进行定义, 其中 $\{\mathbf{x}: h(\mathbf{x}) \neq f(\mathbf{x})\}$ 表示所有预测错误的 feature 的集合 (可以理解为一个事件), 而 $\mathcal{D}(\mathbf{x}: h(\mathbf{x}) \neq f(\mathbf{x}))$ 表示这个事件发生的概率, 即错误集合的测度.
...
- 例如上述背住训练集的反例, 其在训练集上的 empirical loss 是 0, 但是其 almost surely 在 population loss 上得到错误预测.
...
- 这里额外定义"问题" hypothesis 为 $\mathcal{H}$ 中所有 population loss 大于 $\epsilon$ 的 predictor, 即
    $$
...
- 下面继续对 RHS 的每个给定的 $h \in \mathcal{H}_B$ 的 $\mathcal{D}^m(\{S|_{\mathbf{x}} : L_S(h) = 0\})$ 分析其上界.
  - 对于泛化失败的 hypothesis $h \in \mathcal{H}_B$, 其在训练集上的误差 $L_S(h) = 0$ 等价于 $\forall i \in [m], h(\mathbf{x}_i) = f(\mathbf{x}_i)$, 因此由于数据是 i.i.d. 的, 有
    ...
    $$
  - 对固定 $h \in \mathcal{H}_B$, 其 population loss 大于 $\epsilon$, 即任意预测错误的概率大于 $\epsilon$, 因此反过来预测正确的概率小于 $1 - \epsilon$, 即
    $$
