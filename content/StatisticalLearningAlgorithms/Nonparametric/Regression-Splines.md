...
aliases:
  - "Regression_Splines"
  - "Regression Splines"
  - "StatisticalLearningAlgorithms/Nonparametric.Regression-Splines"
...
- **Piecewise polynomials** fit low-degree polynomials over different regions of $X$.

### 1.1 Knots

- Knots are the cut points that divide the range of $X$ into distinct regions.

**Example (Piecewise cubic polynomial with one knot at $c$).**
...
- More knots bring more flexibility.
- A piecewise constant [Step Functions](./Step-Functions.md) are polynomials of degree $0$.

![Piecewise polynomial illustration](https://michael-1313341240.cos.ap-shanghai.myqcloud.com/202401230026293.png)
...
- Each additional constraint reduces one degree of freedom.
- If we constrain the piecewise polynomials, e.g. cubic, to be:
  - continuous at the knots;
  - continuous for the first derivative at the knots;
  - continuous for the second derivative at the knots;

  then we call it a **cubic spline**.

**Example (Cubic spline).** In this case, each region has 4 parameters, thus 8 parameters in total. We have 3 constraints: continuity, continuity of the first derivative, and continuity of the second derivative. Thus 3 degrees of freedom are reduced.

In general, if we have $K$ knots, the degree of freedom is $K+4$.
...
Spline basis representation helps with fitting the model under the constraints.
...
- It can be easily proved that the truncated power basis function per knot remains continuous up to the second derivative at the knot.
- Thus to fit a data set with $K$ knots, we perform LSE with $K+4$ predictors, including the intercept: $X, X^2, X^3, h(X, \xi_1), h(X, \xi_2), \cdots, h(X, \xi_K), \beta_0$.

![Spline basis illustration](https://michael-1313341240.cos.ap-shanghai.myqcloud.com/202401230130691.png)
...
- Natural splines generally produce more stable estimates at the boundaries, with smaller variance.
...
- Another option is to place knots in a uniform fashion, i.e. specify the desired degrees of freedom of the spline, and then have software automatically place the corresponding number of knots at uniform quantiles of the data.

### 4.2 Choosing the Number of Knots
...
> [!note] Note: Polynomial regression comparison
> The source note has a heading for comparison with polynomial regression, but does not include the comparison content. It is left as a planned expansion.

## Related Notes

- [Step Functions](./Step-Functions.md)
- [Smoothing Splines](./Smoothing-Splines.md)
