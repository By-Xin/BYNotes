---
title: "05 - Recurrent Neural Networks"
aliases:
  - "Topic 05 · Recurrent Neural Networks"
...
> [!quote] References
>
> - Reading: Dive into Deep Learning (D2L), Recurrent Neural Networks
...
- 输入输出都是不定长序列 (Sequence-to-Sequence). 具体而言有两种情况:
  ...
  - Unaligned: 每个步骤的输入输出顺序不一定对应
    - 如: 英文的 _Go to school_ 在日语中可能是 _学校に行く_ (学校: school, 行く: go), 其主语和动词的顺序是不一样的.

RNN 的研究和 NLP 的发展是相辅相成的.

模型的输入是一个序列 $\mathbf{x} = (\mathrm{x}_1, \mathrm{x}_2, \ldots, \mathrm{x}_T)$, 其中 $\mathrm{x}_t \in \mathbb{R}^d$ 表示第 $t$ 个时间步的输入. 我们可以认为这个序列是独立得到的, 但是对于序列中的每个元素往往是有关联的. 因此我们往往会联合的考虑整个序列的密度分布:

$$
p(\mathbf{x}) = p(\mathrm{x}_1, \mathrm{x}_2, \ldots, \mathrm{x}_T)
$$

即尝试着去建模得到这个序列的联合概率分布.
...
- 另一种方法是 _Latent Autoregressive Model_. 具体而言, 我们通过引入一个 latent variable $h_t$ 来概括截止到时间步 $t-1$ 的全部历史信息, 即 $h_t = g(x_{t-1}, h_{t-1})$. 这样我们就可以通过 $h_t$ 来预测 $x_t$.
...
## 2. Converting Raw Text into Sequence Data
...
### 2.1 Exploratory Language Statistics
...
我们往往会利用 Markov 假设来简化模型, [[N-Gram-Language-Models|不同 order 的 Markov model 会对应不同的 N-gram model]]:

- Unigram Model / 零阶 Markov Model: $\mathbb{P}(x_1, x_2, \ldots, x_T) = \prod_{t=1}^T \mathbb{P}(x_t)$
- Bigram Model / 一阶 Markov Model: $\mathbb{P}(x_1, x_2, \ldots, x_T) = \mathbb{P}(x_1) \prod_{t=2}^T \mathbb{P}(x_t | x_{t-1})$
- Trigram Model / 二阶 Markov Model: $\mathbb{P}(x_1, x_2, \ldots, x_T) = \mathbb{P}(x_1) \mathbb{P}(x_2 | x_1) \prod_{t=3}^T \mathbb{P}(x_t | x_{t-1}, x_{t-2})$

### 3.1 Word Frequency for Language Models
...
RNN 沿用了 latent autoregressive model 的思想, 通过引入一个 hidden state $h_t$ 来概括选定时间窗口的历史信息. 具体而言, 我们有:
...
> ! 事实上 one-hot 并不是最好的输入方式 (其与index 等价, 都是一种稀疏表示). [[Word2Vec|在后面还会介绍 word embedding 的方式来表示输入]].
...
为了解决 RNN 的梯度爆炸问题, 一个简单的方法是梯度裁剪 (Gradient Clipping). 粗略地说, 我们需要强制 gradient 的范数不超过一个阈值 $\theta$.

[[5.Gradient-Descent|考虑最简单的 GD 算法, 我们有]]:
...
- 更理想的方法是 gradient clipping, 即尝试控制梯度 $\|\mathbf{g}\|$ 的取值. 具体而言, 我们在更新参数之前, 先计算梯度及其范数, 如果梯度的范数超过了阈值 $\theta$, 我们就将梯度的范数缩放到 $\theta$:
  $$
...
- [Attention and Transformer](./7.Attention-and-Transformer.md)
