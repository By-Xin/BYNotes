---
title: "04 - Modern CNNs"
aliases:
  - "Topic 04 · Modern CNNs"
  - "Modern_CNNs"
  - "Modern CNNs"
  - "AlexNet"
  - "VGG"
...
> [!quote] References
>
> - Reading: Dive into Deep Learning (D2L), Modern Convolutional Neural Networks
...
## 1. Missing Ingredients Before AlexNet
...
- AlexNet 使用了 Dropout 来防止过拟合, [[Regularization|而 LeNet 只使用了 weight decay]]
...
- 紧接着对于最后一个 Block 的输出, 由于其有 `num_classes` 个通道, 我们对每个通道都各自进行一次 Global Average Pooling, 也就是对每个通道的 $h\times w$ 的特征图求一个平均值. 因此我们会得到 `num_classes` 个 $1\times 1$ 的特征图 (即每个通道的平均值). 最后我们就可以直接利用这个平均值来进行分类了.
...
Normalization 对于我们并不陌生. 下面给出 Batch Normalization 的基本数学表达. 记 $\mathbf{x}^{(i)} \in \mathcal{B}$ 是当前 batch 中的第 $i$ 个样本, 且 $\mathbf{x}^{(i)} = [x_1^{(i)}, x_2^{(i)}, \ldots, x_d^{(i)}]^\top$ 是一个 $d$ 维的向量.
...
#### 6.1.2 BN 与 Internal Covariate Shift
...
由于 BN 有效控制了每一层的输入分布, 使得每一层的输入分布更加稳定, 因此一定程度上解决了 **internal covariate shift** 的问题.

#### 6.1.3 BN 与 Gradient Explosion/Vanishing
...
- BN 对于 Batch 的大小是敏感的, 因为均值和方差是基于当前 batch 计算的. 譬如对于 `batch_size=1` 的情况, BN 会失效 (因为方差恒为 $0$, 均值退化为该样本自身). 一个经验上推荐的 batch size 是 $50\sim 100$.
...
#### 6.1.6 BN in Fully-Connected Layers
...
#### 6.1.7 BN in Convolutional Layers
...
\left(\widehat\sigma^{(i)}\right)^2 = \frac{1}{d} \sum_{j=1}^{d} \left(x_j^{(i)} - \widehat\mu^{(i)}\right)^2 \in\mathbb{R}
$$
...
此外, 由于需要进行残差连接, 因此需要注意数据的维度, 从而使得 $\mathbf{x}$ 和 $\mathbf{g(\mathbf{x})}$ 的维度一致, 从而可加. 例如下图, $1\times 1$ 卷积层的作用就是将输入的维度进行调整, 使得其与输出的维度一致.
...
ResNet 的实现和 GoogLeNet 类似, 但其参数更少, 架构更简洁, 也因此更受欢迎.

## Related Notes

- [Convolutional Neural Networks](./2.Convolutional-Neural-Networks.md)
- [CNN Architecture](./3.CNN-Architecture.md)
