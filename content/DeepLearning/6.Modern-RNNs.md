...
RNN 在长序列上, 由于其梯度会层层累加, 造成 gradient explosion 或 gradient vanishing 问题. 因此我们需要一些更复杂的结构来维护长时间的依赖关系. LSTM 通过引入 _Gate_ 来控制信息的流动, 使得信息可以在 cell state 中长期存储, 有效的解决了 Vanishing gradient 问题.
