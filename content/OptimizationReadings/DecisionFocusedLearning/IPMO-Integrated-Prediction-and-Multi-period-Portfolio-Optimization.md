---
title: "IPMO: Integrated Prediction and Multi-period Portfolio Optimization (arXiv, 2025)"
aliases:
  - "IPMO"
...
tags:
  ...
  - topic/multi-period-optimization
---

## Citation

Qi Deng, Yuxuan Linghu, and Zhiyuan Liu, "Integrated Prediction and Multi-period Portfolio Optimization," arXiv:2512.11273, 2025.

[Paper](https://arxiv.org/abs/2512.11273)

## Abstract

This paper studies multi-period portfolio optimization as a decision-focused learning problem. Instead of training a return predictor only for forecast accuracy and then passing forecasts into a downstream optimizer, IPMO trains the predictor through the portfolio objective itself. The model uses multi-period return forecasts to parameterize a differentiable mean-variance optimization layer with turnover penalties, then learns from realized portfolio performance. Its main computational device is a mirror-descent fixed-point differentiation scheme, which avoids directly factorizing KKT systems and keeps implicit-gradient computation scalable as the planning horizon grows.
...
这里进一步给出 _time-separable structure_ 的效用函数的定义, 以便后续的分析. 其将负效用函数(之期望)定义为:
...
_Proof_. 对于全局最优解 $\mathbf{z}^\star = \arg\min_{\mathbf{z} \in \Omega} f(\mathbf{z})$, 其满足 first-order optimality condition, 即 $\langle \nabla f(\mathbf{z}^\star), \mathbf{w} - \mathbf{z}^\star \rangle \geq 0$. 因此考虑 MD 的更新公式
