---
title: "GLinSAT: The General Linear Satisfiability Neural Network Layer By Accelerated Gradient Descent (NeurIPS, 2024)"
aliases:
  - "GLinSAT"
...
tags:
  ...
  - topic/differentiable-optimization
...
Hongtai Zeng, Chao Yang, Yanzhen Zhou, Cheng Yang, and Qinglai Guo, "GLinSAT: The General Linear Satisfiability Neural Network Layer By Accelerated Gradient Descent," _Advances in Neural Information Processing Systems (NeurIPS)_, 2024.

[Paper](https://arxiv.org/abs/2409.17500) · [OpenReview](https://openreview.net/forum?id=m1PVjNHvtP) · [Code](https://github.com/HunterTracer/GLinSAT)

## Abstract

GLinSAT is a differentiable neural-network layer that maps model outputs to solutions satisfying bounded general linear constraints. It reformulates the projection as an entropy-regularized linear program, converts the constrained primal problem into a smooth unconstrained dual problem, and solves it using an adaptive accelerated gradient method without matrix factorization. Gradients can be obtained either by differentiating through the iterations or by implicit differentiation with a conjugate-gradient solve. The method is evaluated on constrained traveling-salesman, graph-matching, portfolio-allocation, and unit-commitment tasks.
...
- 对于这个目标函数的理解如下.
  ...
  - 对应的标量函数 (延拓定义 $\phi(0) = 0\log 0 :=0$) (注意到其恰为 Bernoulli 的负 entropy)
    ...
    其图象如下所示.
    ![Bernoulli negative entropy regularizer as a function of the normalized variable](https://raw.githubusercontent.com/By-Xin/Blog-figs/main/phi_p.png)

    _Figure: The Bernoulli negative entropy regularizer favors smooth interior solutions._
    则正则项可以表达为:
...
- 给定 $F$ 的 convex & Lipschitz 光滑性质, 因此可以使用 Nesterov 型的 primal-dual 梯度加速方法 (**A**daptive**P**rimal**D**ual**A**ccelerated**G**radient**D**escent) 对这个目标函数进行优化, 以得到 $\mathcal{O}(1/k^2)$ 的收敛率.
  ...
  - 下面具体讨论一下 GLinSAT 的 Nesterov 加速算法. 其主体同样采用三阶段的 Nesterov 加速方法:
    ...
    - 最后, 进行自适应的尝试更新 Lipschitz 常数 $M^{k+1}$. 观察上面的等式, $1/M^k$ 是更新的步长. 如果 $M^k$ 太大, 则步长太小, 更新过于保守. 因此我们需要随着迭代的不断进行, 不断尝试得到一个更小更为准确的曲率系数 $M^k$, 以得到更为合理的步长. 事实上, 我们进行尝试的选择依据就是 $F(\mathbf y) \le F(\mathbf x)+\left\langle\nabla F(\mathbf x),\mathbf y-\mathbf x\right\rangle+\frac{M}{2}\|\mathbf y-\mathbf x\|_2^2$. 将具体的迭代点代入, 最终的 linesearch 的条件可以化简整理为:
      ...
      - 当然在实践中还会有一些工程参数, 例如额外引入 tolerance 阈值 $\delta>0$. 当 $F(\boldsymbol{\eta}^{k+1}) > F(\boldsymbol{\lambda}^{k+1}) - \frac{1}{2M^k} \|\nabla F(\boldsymbol{\lambda}^{k+1})\|^2 + \delta$ 时, 说明当前 $M^k$ 仍然过小, 应拒绝该次尝试并增大 $M^k$. 当充分下降条件连续成功两次时, 才正式尝试更新 $M^{k+1} = M^k / 2$. 这样可以避免过于频繁地更新 $M^k$ 而导致不稳定.

![Adaptive primal-dual accelerated gradient descent algorithm used in the GLinSAT forward pass](https://raw.githubusercontent.com/By-Xin/Blog-figs/main/20260624213825.png)

_Figure: APDAGD updates the dual variables while maintaining an averaged primal solution._
...
- 回顾, 优化的对偶目标为
  ...
  $$
  \mathbf{h}(\mathbf{y}, \mathbf{c}) := \mathbf{A} \mathbf{x}(\mathbf{y}, \mathbf{c}) - \mathbf{b}
  = \mathbf{A} \left(\mathbf{u} \odot \sigma(-\theta \mathbf{u} \odot (-\mathbf{A}^\top \mathbf{y} - \mathbf{c}))\right) - \mathbf{b},
  $$

  并且在最优点处满足 $\mathbf{h}(\mathbf{y}^\star, \mathbf{c}) = 0$.
