---
title: "Contextual Optimization Under Model Misspecification: A Tractable and Generalizable Approach (ICML, PMLR, 2025)"
aliases:
  - "Contextual Optimization Under Model Misspecification: A Tractable and Generalizable Approach"
  - "Contextual Optimization Under Model Misspecification"
type: "paper-note"
status: "draft"
tags:
  - reading/papers
  - topic/decision-focused-learning
  - topic/contextual-optimization
  - topic/model-misspecification
---

## Citation

Omar Bennouna, Jiawei Zhang, Saurabh Amin, and Asuman E. Ozdaglar, "Contextual Optimization Under Model Misspecification: A Tractable and Generalizable Approach," _Proceedings of the 42nd International Conference on Machine Learning_, PMLR 267:3749-3775, 2025.

[Paper](https://proceedings.mlr.press/v267/bennouna25a.html) · [PDF](https://raw.githubusercontent.com/mlresearch/v267/main/assets/bennouna25a/bennouna25a.pdf) · [OpenReview](https://openreview.net/forum?id=e3NNvqD7wA)
...
下面正式给出问题 formulation. 在观察到某个具体的 context $\mathbf{x}$ 后, 我们希望求解如下的决策问题:
$$
...
### 2.1 Rewriting target loss under uniqueness assumption

文章指出, $\mathcal{W}^\star(\mathbf{\hat{c}}_\theta(\mathbf{x}))$ 尽管理论上可能多解, 但实践中, 若 $\mathbf{\hat{c}}_\theta(\mathbf{x})$ 是连续随机变量, 且 $\mathcal{W}$ 是 polyhedron, 则往往具有唯一解. 因此文章将唯一性单独列为 Assumption 1: 对任意 $\theta \in \mathbb{R}^m$, 若 $\mathbf{\hat{c}}_\theta(\mathbf{x}) \neq 0$ almost surely, 则 $\min_{\mathbf{w} \in \mathcal{W}} \mathbf{\hat{c}}_\theta(\mathbf{x})^\top \mathbf{w}$ 在 $(\mathbf{x}, \mathbf{c}) \sim \mathcal{P}$ 下以概率 $1$ 有唯一解. 需要注意, $\mathbf{\hat{c}}_\theta(\mathbf{x}) \neq 0$ 本身并不足以推出唯一性; 它必须与 Assumption 1 一起使用.
...
### 2.2 Surrogate loss: Consistent Integrated Learning and Optimization (CILO)
...
因此这个 loss 的意义在于, 在限制 policy 为真实 cost 不超过 $\beta$ 的 subset 中, 其在预测 cost 下的表现与在所有 policy 中的表现的 gap. 在下面 Theorem 1 的条件下, 有关系:
$$
\ell_\mathcal{P}^\beta(\theta) = 0 \iff \ell_\mathcal{P}(\theta) \leq \beta
$$
论文进一步证明, 当 $\beta \geq \beta^\star_{\mathcal{H}, \mathcal{P}}$ 时, $\ell_\mathcal{P}^\beta(\theta) \geq 0$. 因此在 Theorem 1 的条件下, $\ell_\mathcal{P}^\beta(\theta) = 0$ 又等价于 $\theta$ 是 $\ell_\mathcal{P}^\beta$ 的 minimizer. 因此有如下定理:

> [!theorem] Theorem 1
>
> 令 $\beta \in \mathbb{R}$ 满足 $\beta^\star_{\mathcal{H}, \mathcal{P}} \leq \beta < \beta_{\max, \mathcal{P}}$. 在 Assumption 1 下, 对任意满足 $\mathbf{\hat{c}}_\theta(\mathbf{x}) \neq 0$ almost surely 的 $\theta \in \mathbb{R}^m$, 有 $\ell_\mathcal{P}(\theta) \leq \beta$ 当且仅当 $\theta$ 是 $\ell_\mathcal{P}^\beta$ 的 minimizer. 特别地, 当 $\beta = \beta^\star_{\mathcal{H}, \mathcal{P}}$ 时, $\theta$ 是 $\ell_\mathcal{P}^\beta$ 的 minimizer 当且仅当它是 $\ell_\mathcal{P}$ 的 minimizer.
...
### 3.1 Generalization Guarantee


为确保泛化性, 做如下假设:

- 此处不要求 hypothesis set 具有线性形式; 只假设 predictor 由 $\theta \in \mathbb{R}^m$ 参数化, 并满足下面的 boundedness 和 regularity 条件.
...
- 注意区分 $\beta_{\min, \mathcal{P}}$ 与 $\beta^\star_{\mathcal{H}, \mathcal{P}}$, 前者是 oracle 下界, 后者是当前 hypothesis class $\mathcal{H}$ 能够达到的最优真实 decision loss. 故恒有 $\beta_{\min, \mathcal{P}} \leq \beta^\star_{\mathcal{H}, \mathcal{P}}$.
  - 若 $\mathcal{H}$ 是 well-specified 的, 则确实能够产生 oracle-optimal 的决策, 故 $\beta_{\min, \mathcal{P}} = \beta^\star_{\mathcal{H}, \mathcal{P}}$.
...
### 3.2 Stronger Consistency Guarantee
...
假设: 
1. 只要 $\theta \neq 0$, 则 $\mathbf{\hat{c}}_\theta(\mathbf{x}) \neq 0$ a.s. 成立. 这结合之前的 Assumption 1 保证 $\min_{\mathbf{w} \in \mathcal{W}} \mathbf{\hat{c}}_\theta(\mathbf{x})^\top \mathbf{w}$ 的唯一性成立.
...
> [!theorem] Theorem 3
>
> 在 Assumptions 2, 3 和 5 下, 存在 $\alpha > 0$, 使得对于任意固定的 $u > 0$、任意满足 $\|\theta\|_2 \geq u$ 的 $\theta \in \mathbb{R}^m$, 以及任意 $\beta \geq \beta^\star_{\mathcal{H}, \mathcal{P}}$, 有
>
> $$
...
假设 $\mathbf{\hat{c}}_\theta(\mathbf{x})$ 是一个连续函数, 且 $\nabla_\theta \mathbf{\hat{c}}_\theta(\mathbf{x})$ 是 $B_L > 0$ Lipschitz continuous 的, 则可以证明这两个函数都是 weakly convex 的. 通过给二者添加同一个足够大的二次项, 可以将它们分别写成 convex functions, 因此 $\ell_{\mathcal{P}_n}^\beta(\theta)$ 是一个 DC function.

这里又重新进行了一次 rename: $g^1_{\mathcal{P}_n}(\theta) := g_{\mathcal{P}_n}(\theta)$, $g^2_{\mathcal{P}_n}(\theta) := \bar{g}^\beta_{\mathcal{P}_n}(\theta)$, 则 $\ell_{\mathcal{P}_n}^\beta(\theta) = g^1_{\mathcal{P}_n}(\theta) - g^2_{\mathcal{P}_n}(\theta)$. 此时对每个函数进行 Moreau Smoothing:
$$
M_{\mathcal{P}_n}^i(\lambda) := \min_{\theta \in \mathbb{R}^m} \left\{ g^i_{\mathcal{P}_n}(\theta) + \frac{1}{2} \|\lambda - \theta\|_2^2 \right\}, \quad i = 1, 2
$$
以及
$$
...
为得到后续的集合表示, 下面进一步假设 hypothesis set 是线性的: 存在 $\Phi : \mathbb{R}^k \to \mathbb{R}^{d \times m}$, 使得 $\mathbf{\hat{c}}_\theta(\mathbf{x}) = \Phi(\mathbf{x})\theta$.

下面讨论如何避免出现 $\theta = 0$ 的情况. 定义如下的 log-CILO loss:
$$
f_{\mathcal{P}_n}^\beta(\lambda) := \log d(\lambda, - V_{\mathcal{P}_n}^\beta) - \log d(\lambda, - V_{\mathcal{P}_n})
$$
其中
$$
...
$$
$$
V_{\mathcal{P}_n}^\beta = \{ \mathbb{E}_{(\mathbf{x}, \mathbf{c}) \sim \mathcal{P}_n} \left[ \Phi(\mathbf{x})^\top \bar{w}_{\mathcal{P}_n}^\beta(\mathbf{x}) \right] : \bar{w}_{\mathcal{P}_n}^\beta \in \mathcal{\bar{W}}_{\mathcal{P}_n}^\beta \}
$$
前者对应所有 policies, 后者对应所有在真实 cost 下 performance 不超过 $\beta$ 的 policies. $d$ 是欧氏距离. 

由于 $V_{\mathcal{P}_n}^\beta \subseteq V_{\mathcal{P}_n}$, 有 $d(\lambda, -V_{\mathcal{P}_n}^\beta) \geq d(\lambda, -V_{\mathcal{P}_n})$. 因而在两个距离均为正的定义域上恒有 $f_{\mathcal{P}_n}^\beta(\lambda) \geq 0$, 且两个距离相等时 $f_{\mathcal{P}_n}^\beta(\lambda) = 0$. 论文进一步证明, log-CILO 的 stationary point 也是 s-CILO 的 stationary point, 反向关系则不由该结论保证.

总结下来, log-CILO 用于引导优化过程避开对应 $\theta = 0$ 的共同边界情形. 完整方法还需要结合 s-CILO 上的 gradient descent 与论文 Theorem 6 的构造; 单独优化 log-CILO 并不能无条件保证 $\theta \neq 0$.
